...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
		...
		public void Move(Vector3 move, bool crouch, bool jump, bool _RunTurnAround)
		{
            RunTurnAround = _RunTurnAround;
            ...
            if(m_TurnAmount > 0.9 /*&& m_ForwardAmount > 0.5*/)
            {
               turnAround("Right");
            }else if(m_TurnAmount < -0.9 /*&& m_ForwardAmount > 0.5*/)
            {
                turnAround("Left");
            }
		...
		void detectWallsAndIdle(float currentSpeed){    
			Vector3 fwd = transform.TransformDirection(Vector3.forward);
            if (Physics.Raycast(transform.position, fwd, 1f ) && m_IsGrounded)
            {
		...
		void UpdateAnimator(Vector3 move)
		{
            ...
            currentForward = m_Animator.GetFloat("Forward");

            if (m_Animator.GetFloat("Forward") < 1 && m_Animator.GetFloat("Forward") > 0.7f)
            {
                detectWallsAndIdle(currentForward);
            }
